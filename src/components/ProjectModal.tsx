  ...
  return (
    <>
                ...
                <div className="grid lg:grid-cols-[1.1fr_0.9fr]">
                  <div className="border-b border-white/10 lg:border-b-0 lg:border-r lg:border-white/10">
                    ...
                    {project.gallery.length > 0 && (
                      <div className={`grid gap-3 p-4 ${project.gallery.length === 1 ? "grid-cols-1" : "grid-cols-2 md:grid-cols-4"}`}>
                        {project.gallery.map((img, index) => (
                          <button
                            key={index}
                            type="button"
                            onClick={() => setZoomedImage(img)}
                            className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] text-left"
                          >
                  ...
                  <div className="p-6 md:p-8">
                    ...
                    {project.liveUrl && (
                      ...
                      >
                        {project.buttonLabel || "Visit site"} <ArrowUpRight size={16} />
                      </a>
                    )}
                  </div>
                </div>
      ...
      <AnimatePresence>
        {zoomedImage && (
          <motion.div
            className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 p-6 backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            >
              <button
                onClick={() => setZoomedImage(null)}
                className="absolute right-4 top-4 z-20 flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/80 transition hover:bg-white/10 hover:text-white"
              >
                <X size={18} />
              </button>
